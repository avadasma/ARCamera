...
public class ARCamPresenter implements ARCamContract.Presenter {
    ...
    @Override
    public void handlePhotoFrame(byte[] bytes, Bitmap mRajawaliBitmap, int photoWidth,
                                 int photoHeight) {
        // The camera preview frame data into Bitmap
        Bitmap bitmap = Bitmap.createBitmap(photoWidth, photoHeight, Bitmap.Config.ARGB_8888);
        ByteBuffer b = ByteBuffer.wrap(bytes);
        bitmap.copyPixelsFromBuffer(b);
        // Rajawali rendered 3D model not empty screenshot
        if (mRajawaliBitmap != null) {
            Log.i(TAG, "mRajawaliBitmap != null");
            mRajawaliBitmap = Bitmap.createScaledBitmap(mRajawaliBitmap, photoWidth, photoHeight,
                    false);
    ...
    public void handleChangeModel(float[] landmarkX, float[] landmarkY) {
        ...
        mDynamicPoints.add(new DynamicPoint(20, (landmarkX[36] + landmarkX[39]) * 0.5f,
                (landmarkY[36] + landmarkY[39]) * 0.5f, 0.0f));
